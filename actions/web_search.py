...
def _из_выдержек(query: str, mode: str, результаты: list[tuple[str, str]]) -> str:
    """Ответ по выдержкам поисковика — словами, а не склейкой заголовков.

    Склейка «Заголовок. Выдержка Заголовок. Выдержка» — это стена SEO-текста,
    зачитанная вслух: в ней есть ответ, но найти его на слух нельзя. Здесь
    выдержки отдаются текстовой модели с прямым запретом добавлять что-либо
    от себя: она нужна как пересказчик, а не как источник.

    Gemini пропускается: его только что спросили и он отказал, а повторная
    попытка стоит ещё одного таймаута на пути, который и так деградировал.
    Зато локальная модель, если она есть, справится с этим прекрасно.

    Пустая строка — значит пересказать некому, и вызывающий склеит сам.
    """
    from core import llm_client

    выдержки = "\n".join(
        f"- {заголовок}. {выдержка}" if выдержка else f"- {заголовок}"
        for заголовок, выдержка in результаты
    )
    указание = _MODE_INSTRUCTIONS.get(mode, _MODE_INSTRUCTIONS["search"])

    return llm_client.ask(
        f"Вопрос: {query}\n\nВыдержки из поисковой выдачи:\n{выдержки}",
        система=(
            "Ты отвечаешь на вопрос ТОЛЬКО по выдержкам ниже. Ничего не "
            "добавляй от себя и не досказывай по памяти: если ответа в "
            "выдержках нет, так и скажи одной фразой. Отвечай по-русски, "
            "обращайся «сэр», без markdown и списков — это читают вслух. "
            f"{указание}"
        ),
        предел=400,
        кроме=("gemini",),
...
def web_search(parameters: dict, player=None) -> str:
    ...
    if результаты:
        пересказ = _из_выдержек(query, mode, результаты)
        if пересказ:
            return _shorten(пересказ)
        # Пересказать некому — отдаём как есть. Хуже связного ответа, но
        # лучше молчания: ответ в этих строчках всё-таки есть.
