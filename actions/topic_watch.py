...
def topic_watch(parameters: dict, player=None) -> str:
    ...
    уведомлять = параметры.get("notify")
    if isinstance(уведомлять, str):
        уведомлять = уведомлять.strip().lower() in ("true", "да", "1", "yes")
    ...
    if действие in _СЕЙЧАС:
        ...
        новости, нашлась = watcher.проверить_по_просьбе(тема)
        if not нашлась:
    ...
    return watcher.add(тема, режим, интервал, bool(уведомлять))
...
TOOL = {
    ...
    "description": (
        ...
        "Если просят «напомни, когда», «дай знать, даже если меня нет» — "
        "ставь notify=true: тогда новость покажется уведомлением системы, а "
        "не только прозвучит. "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "notify": {
                "type": "BOOLEAN",
                "description": (
                    "Показывать ещё и уведомление системы — чтобы новость не "
                    "пропала, если человек отошёл от компьютера"
                ),
            },
        },
        "required": [],
