...
def _пересказать(текст: str, вопрос: str, имя: str) -> str:
    ...
    try:
        ...
        ответ = клиент.models.generate_content(
            ...
            contents=(
                "Ниже содержимое файла. Ответь по-русски, обращайся «сэр», "
                "четыре-шесть предложений, без markdown и списков — это читают "
                "вслух. Не добавляй ничего, чего в файле нет; если ответа там "
                "нет, так и скажи.\n\n"
                f"Задача: {задача}\nФайл: {имя}\n\nСодержимое:\n{текст}"
            ),
        ...
        _logger.warning("Пересказ файла не вышел: %s", exc)
        return ""
