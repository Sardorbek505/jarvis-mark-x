...
Цепочка одного хода:
    ...
    последний громкий кадр микрофона
        │  (сеть + распознавание)
        ├──> последний фрагмент расшифровки    "слышит"
...
class LatencyTracker:
    ...
    def mark_transcript(self) -> None:
        """Пришёл фрагмент расшифровки сказанного.

        Запоминается ПОСЛЕДНИЙ: расшифровка идёт по слогам ещё во время речи,
        и отсчёт от первого куска давал отрицательное «слышит» (стенд 16.09.2026:
        −939 мс). «Слышит» — момент, когда фраза распознана целиком.
        """
