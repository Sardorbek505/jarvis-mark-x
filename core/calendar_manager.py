...
def _parse_datetime(text: str, reference_date: Optional[datetime] = None) -> Optional[datetime]:
    ...
    # Было ли в тексте вообще что-то про время.
    #
    # Функция объявлена как `Optional[datetime]`, и все три вызывающих места
    # проверяют результат на None — но получить None было невозможно: любая
    # непонятная фраза доезжала до «сегодня в 9:00» по умолчанию. То есть
    # «напомни когда-нибудь» молча ставило напоминание на девять утра, а
    # «добавь встречу» без времени — встречу на утро сегодняшнего дня.
    # Проверки existed, а защиты не было.
    _МАРКЕРЫ = (
        'через', 'сегодня', 'завтра', 'послезавтра', 'вчера', 'позавчера',
        'на следующей неделе', 'утром', 'днём', 'днем', 'вечером', 'ночью',
        'понедельник', 'вторник', 'среда', 'среду', 'четверг', 'пятница',
        'пятницу', 'суббота', 'субботу', 'воскресенье',
    )
    if not any(м in text for м in _МАРКЕРЫ) and not re.search(r'\d{1,2}[:.]\d{2}', text):
        return None
