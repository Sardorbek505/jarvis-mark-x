...
class HeadlessUI:
    ...
    # Экранного подтверждения здесь нет и быть не может: нажимать некому.
    # Флаг читает Jarvis и не привязывает к этому интерфейсу core/confirm.py —
    # необратимые действия тогда проходят через запасной гейт с повторным
    # вызовом. Методы ниже всё равно обязаны существовать: без них запуск без
    ...
    def bind_push_to_talk(self, on_change) -> None:
        """Окна нет — ловить аккорд нечем.

        Метод обязан существовать: Jarvis обращается к нему при включённом
        режиме, и «нет такого метода» здесь означало бы падение запуска без
        окна (см. tests/test_headless.py). Само отсутствие режима — не беда:
        в headless и клавиатуры может не быть."""
        _logger.info("Режим «зажми и говори» без окна недоступен — микрофон открыт как обычно")

    def show_confirm(self, title: str, detail: str = "") -> None:
        """Показать баннер негде — пишем в лог, чтобы след остался."""
        _logger.warning("подтверждение без окна: %s (%s)", title, detail)
