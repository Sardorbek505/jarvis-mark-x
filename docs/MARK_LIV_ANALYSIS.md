...
13. ✅ **Сделано:** `actions/file_processor.py` — читает текст, код, JSON, PDF,
    Word, Excel и картинки и отвечает по содержимому. Плюс перетаскивание файла
    в окно: `_on_file` в `ui.py` существовал, но его никто не вызывал.
14. ✅ **Сделано:** `core/browser_session.py` на Playwright — нажатие, ввод и
    чтение страницы в отдельном автоматизируемом окне; «открой сайт» по-прежнему
    уходит в браузер пользователя. Единственная часть всей работы, проверенная
    вживую: Chromium в контейнере есть.
15. ✅ **Сделано:** `actions/youtube_video.py` — пересказ ролика по субтитрам,
    не открывая его. Тренды сознательно не сделаны.
16. ✅ **Сделано:** 43 действия в `computer_control` вместо девяти, все названы
    в объявлении, опечатки и русские синонимы чинит `difflib` локально.
17. ✅ **Сделано:** `plugins/` — продолжение реестра, а не второй загрузчик.
    Выключение через `plugins_disabled`, имена штатных инструментов заняты.

### Уровень D — остаток списка, взят целиком

Двенадцать пунктов, оставшихся после уровня C, закрыты. Порядок — по
соотношению «польза на единицу работы», а не по номерам.

18. ✅ **Мгновенное подтверждение** (`core/acknowledge.py`). Перед медленным
    инструментом звучит короткая фраза, и человек больше не слышит четыре
    секунды тишины вместо ответа. Ни одна фраза не обещает результата, быстрые
    инструменты молчат, а если модель уже что-то сказала в этом ходе —
    подтверждения нет.
19. ✅ **Пять действий `files`**: `append` (дописать, не стерев), `info`,
    `largest`, поиск по расширению и `organize` (раскладка по папкам). Отмена
    дописывания — обрезкой до прежней длины, вся раскладка отменяется одним
    «отмени».
20. ✅ **Слежение за темой** (`core/watcher.py` + `actions/topic_watch.py`).
    Главное здесь — когда Джарвис МОЛЧИТ: пересказ той же новости другими
    словами не новость, пять тем, не чаще раза в четверть часа, ни слова с 23
    до 8.
21. ✅ **Панель памяти** в окне: увидеть накопленное и забыть лишнее.
    `all_entries()` и `forget()` были написаны раньше и ждали применения.
22. ✅ **Сообщения в мессенджер** (`actions/messenger.py`). Переписка
    открывается с готовым текстом, отправку нажимает человек — и в ответе
    прямо сказано, что не отправлено. Номер не угадывается.
23. ✅ **Рабочий стол** (`actions/desktop.py`): обои по брошенной в окно
    картинке и «сверни всё». Прежние обои читаются ДО замены, поэтому «отмени»
    возвращает именно их.
24. ✅ **Запуск скриптов** (`actions/code_runner.py`) — за той же кнопкой, что
    выключение компьютера, плюс разбор Python без запуска. Написания кода нет:
    его пишет модель, а кладёт в файл `files`.
25. ✅ **Буфер обмена** — инструментом, а не всплывающей панелью. Следить за
    буфером значит читать все пароли, которые человек копирует; сценарий
    «переведи, что я скопировал» сохранён целиком.
26. ✅ **Провайдерный слой** (`core/llm_client.py`) — только для ТЕКСТОВЫХ
    задач, и в описании так и написано. Голосовой ход живёт внутри Live API;
    выносить его — это второй турн-цикл с собственным VAD, та самая тяжёлая
    часть, которую Mark-XLIX бросил. Зато кончившаяся квота больше не отнимает
    пересказ там, где стоит Ollama.
27. ✅ **Настройки разговора** в окне: окно VAD, захват начала фразы,
    размышления, показ замера. Попутно закрыта ловушка, которую эта же ручка и
    создавала: хвост тишины микрофона теперь едет за окном VAD, иначе ползунок
    «пусть не перебивает» сделал бы Джарвиса немым.
28. ✅ **Мышь по описанию словами** (`actions/screen_pointer.py`): «нажми
    Сохранить». Сомневается — не нажимает, и это правило, а не недоделка.
29. ✅ **Резюме разговора** (`core/session_log.py`): «вчера мы говорили о…» в
    брифинге и по прямому вопросу. Запись дешёвая, выжимка ленивая, хранение
    две недели.

### Уровень E — не брать

30. **Аватар с виземами** (`avatar.py` + `avatar_mesh.py` + `viseme.py` +
...
31. **`game_updater`** (Steam/Epic, 1101 строка) — обновление и установка игр.
    Остаётся «не брать»: гигабайты по чужой воле и закрытие игры процессом —
    не то, что делают по фразе, услышанной в комнате.

    **Но по отдельной просьбе взят ЗАПУСК** — это другая задача, и куда более
    нужная: `open_app` ищет по PATH, игры там не лежат, и «включи ведьмака»
    не работало вовсе. `actions/game_launcher.py` читает манифесты Steam и Epic
    с диска и передаёт запуск ссылкой лаунчера. Обновления и закрытия в нём
    сознательно нет.
32. **`flight_finder`** — парсинг Google Flights ломается при каждом редизайне;
    поддержка дороже пользы. Наш `web_search` в режиме `price` закроет 80%
    запроса.
33. **`dev_agent`** (пишет проекты целиком) — дублирует то, для чего у
    пользователя уже есть Claude Code/Cursor. Голосом такое не управляют.
34. **`dashboard/server.py`** (телефон через QR + самоподписанный TLS) — у нас
    Telegram-бот и mini-app делают то же самое лучше и без возни с
    сертификатами.
35. **Копирование кода как есть.** CC BY-NC 4.0 против нашего MIT — любая
...
Далее:     B10 ✅, B11 ✅ — уровень B закрыт целиком.
           Уровень C ✅ — закрыт тоже.
           Уровень D ✅ — остаток списка взят целиком, все двенадцать пунктов.
           Не взято только то, что в разделе «не брать»: аватар,
           game_updater, flight_finder, dev_agent и телефонный dashboard.
```

Итог: 35 инструментов, 1035 тестов. Два отступления от списка сделаны
сознательно и объяснены на месте — буфер обмена сделан инструментом, а не
следящей панелью (пункт 25), и провайдерный слой сделан только для текстовых
задач (пункт 26).
