...
**У нас этого нет ни в каком виде** — `grep undo main.py` пуст.
...
**У нас:** `actions/computer_settings.py:221 _power()` вызывает
`shutdown /s /t 5` **сразу, без какого-либо подтверждения**. Одно ослышавшееся
слово — и машина выключается через 5 секунд. Это не «фича Mark-LIV», это наш
баг.
...
**У нас:** `memory/memory_manager.py` — 3 функции, `format_memory_for_prompt()`
вываливает в промпт **всю** память целиком, без бюджета и без обрезки. Пока
фактов мало — работает; на сотне фактов каждый реконнект будет тащить всю
простыню. Ни поиска, ни `forget`, ни индекса ключей у нас нет.
...
**У нас:** psutil используется только в `telegram_bot/pc_server.py` — голосом
спросить «что с компьютером» нельзя.
...
**У нас:** `actions/web_search.py` — 40 строк на DuckDuckGo Instant Answer API
(`api.duckduckgo.com/?q=...&format=json`). Это не поиск, а справочник
определений: на «цена RTX 5080» или «что случилось вчера» он отдаёт пустоту.
Самое слабое место нашего проекта относительно Mark-LIV.
...
|---|---|---|---|
| `action_loader.py` | 221 | Автообнаружение `actions/*.py` по `TOOL` dict + валидация + изоляция падений | Нет — у нас `if name == ...` на 300 строк в `main.py` |
...
| `audio_devices.py` | 421 | Список устройств: 41 запись → 8, один host API на направление, **измерение** работоспособности (запись тишины и замер времени `write`), хранение выбора **по имени**, а не по индексу | Частично: наш `_pick_input_device()` угадывает по названиям («headset», «realtek»), проверяет только «не мёртвое ли». Выход не проверяется вообще |
...
| `hotkey.py` | 173 | Push-to-talk Ctrl+Space: опрос VK-кодов 30 раз/с на Windows, честный fallback в окно на mac/Linux, микрофон **закрыт** пока клавиша не нажата | У нас `hotkey_manager.py` — F8/Ctrl+Shift+J как «разбудить», но не PTT-режим |
...
| Что | Где у них | У нас |
|---|---|---|
| `{токены}` в промпте | `core/prompt.txt` подставляет имя ассистента, ОС, список инструментов, список ограничений на старте сессии | `core/prompt.txt` полностью статичен, токенов нет |
...
### Уровень A — брать обязательно (это закрывает наши дефекты, а не «добавляет фич»)
...
3. **Бюджет памяти + `recall_memory` + индекс `[ALSO REMEMBERED]`.** Сейчас
...
```
Неделя 1:  A2 (resumption) → A1 (confirm) → A5 (requirements)
Неделя 2:  A3 (память: бюджет + recall) → A4 (undo + корзина)
Неделя 3:  B7 (action_loader) → B8 (токены промпта)
Неделя 4:  B6 (web_search) → B9 (system_status) → B12 (PTT)
Далее:     B10, B11, затем C по запросу
```
