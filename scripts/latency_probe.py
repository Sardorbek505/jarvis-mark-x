...
# JARVIS_PROBE_PHRASES="Джарвис, включи музыку|Джарвис, поставь на паузу" —
# свой сценарий через «|», чтобы гонять команды, а не только вопросы.
if os.getenv("JARVIS_PROBE_PHRASES"):
    _PHRASES = {
        f"p{i}": text.strip()
        for i, text in enumerate(os.environ["JARVIS_PROBE_PHRASES"].split("|"))
        if text.strip()
    }

# Пауза между репликами. Должна вмещать ВЕСЬ ход целиком: раздумья, синтез и
# сам ответ вслух. Иначе следующая фраза стартует раньше, чем закончился
# предыдущий ход, точка отсчёта переезжает на неё — и замер выдаёт бессмыслицу
# вроде «отвечает 48 мс». С внешним голосом ход длиннее, отсюда такой запас.
_GAP_SEC = float(os.getenv("PROBE_GAP_SEC", "16"))
